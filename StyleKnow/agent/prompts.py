...
class RouterOutput(BaseModel):
    """Router Agent 输出结构"""
    intent: IntentType = Field(description="用户意图类型")
    reason: str = Field(description="意图识别理由")
    scene: Optional[str] = Field(default="", description="场景关键词")


class CriticOutput(BaseModel):
    """Critic Agent 输出结构"""
    is_pass: bool = Field(description="搭配方案是否通过审核")
    feedback_reason: str = Field(description="审核反馈理由")
...
- update_preference: 用户想要更新个人偏好（如"我不喜欢黑色"）
- chat: 用户只是闲聊或询问问题

请直接输出你的判断，不需要 JSON 格式。"""
...
# ========== 兼容层函数 (已弃用，建议使用 Pydantic Schema) ==========
def parse_intent_response(response: str) -> Dict:
    """
    解析意图识别响应（兼容层，已弃用）
    
    建议使用 RouterOutput Pydantic Model
    """
    import re
    import json
